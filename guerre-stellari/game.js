...
const SPRITES = {
  luke:  { img: new Image(), natFace: 1,  hS: 122, fx: 0.914, fy: 0.472, ar: 361 / 520 },
  vader: { img: new Image(), natFace: -1, hS: 145, fx: 0.086, fy: 0.408, ar: 563 / 520 },
};
...
// punti della lama in coordinate mondo, ancorati alla mano dello sprite
function bladePts(f, S, GY, isVader) {
  ...
  const hx = f.x + flip * (sp.fx - 0.5) * wid;
  const hy = GY + (f.airY || 0) - (1 - sp.fy) * hgt + (isVader ? (f.kneel || 0) * S * 26 : 0);
  const aw = f.face === 1 ? f.armAng : Math.PI - f.armAng;
...
function drawLukeChar(l, S, GY) {
  ...
  if (l.rot) {
    ctx.translate(0, -hgt * 0.45);
    ctx.rotate(l.rot);
    ctx.translate(0, hgt * 0.45);
  }
  if (l.hurtT > 0 && Math.sin(G.time * 40) > 0) ctx.globalAlpha *= 0.55;
  let lean = 0;
  if (l.state === "atk") lean = 0.16 * clamp(l.t / 0.15, 0, 1);
  else if (l.state === "plunge") lean = 0.35;
  else if (l.blocking) lean = -0.06;
  ctx.rotate(lean * l.face);
...
function drawVaderChar(v, S, GY) {
  ...
  ctx.rotate(kneel * 0.22 * v.face);
  let lean = 0;
  if (v.state === "strike") lean = 0.18;
  else if (v.state === "windup" || v.state === "throwTele") lean = -0.1;
  else if ((v.staggerT || 0) > 0) lean = -0.22;
  ctx.rotate(lean * v.face);
