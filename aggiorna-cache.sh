...
#
# La versione è un'impronta del contenuto: cambia solo quando il gioco cambia
# davvero. Dopo la pubblicazione il service worker vede una versione nuova,
# butta la copia salvata sul dispositivo e serve il gioco aggiornato.
...
aggiorna_gioco() {
  ...
  # l'impronta copre nomi e contenuto di tutto ciò che viene servito, sw.js escluso
  elenco=$(find "$gioco" -type f ! -name sw.js | LC_ALL=C sort)
